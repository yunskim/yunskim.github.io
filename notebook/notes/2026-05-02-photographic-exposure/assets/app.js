(() => {
  ...
  function renderExposureDemo() {
    const chart = d3.select("#exposure-chart");
    const chartNode = chart.node();
    const shutterInput = document.querySelector("#exposure-shutter");
    const apertureInput = document.querySelector("#exposure-aperture");
    const isoInput = document.querySelector("#exposure-iso");
    const shutterLabel = document.querySelector("#exposure-shutter-label");
    const apertureLabel = document.querySelector("#exposure-aperture-label");
    const isoLabel = document.querySelector("#exposure-iso-label");
    const values = document.querySelector("#exposure-values");
    if (!chartNode || !shutterInput || !apertureInput || !isoInput || !shutterLabel || !apertureLabel || !isoLabel || !values) return;

    const shutters = [
      { label: "1/4000초", value: 1 / 4000 },
      { label: "1/2000초", value: 1 / 2000 },
      { label: "1/1000초", value: 1 / 1000 },
      { label: "1/500초", value: 1 / 500 },
      { label: "1/250초", value: 1 / 250 },
      { label: "1/125초", value: 1 / 125 },
      { label: "1/60초", value: 1 / 60 },
      { label: "1/30초", value: 1 / 30 },
      { label: "1/15초", value: 1 / 15 },
      { label: "1/8초", value: 1 / 8 },
      { label: "1/4초", value: 1 / 4 },
      { label: "1/2초", value: 1 / 2 },
      { label: "1초", value: 1 },
    ];
    const apertures = [1.4, 2, 2.8, 4, 5.6, 8, 11, 16, 22].map((value) => ({
      label: `f/${value}`,
      value,
    }));
    const isos = [100, 200, 400, 800, 1600, 3200, 6400].map((value) => ({
      label: `ISO ${value}`,
      value,
    }));
    const baseline = { shutter: 1 / 125, aperture: 4, iso: 100 };
    const width = 920;
    const height = 470;
    const margin = { top: 28, right: 186, bottom: 54, left: 126 };
    const innerWidth = width - margin.left - margin.right;
    const innerHeight = height - margin.top - margin.bottom;
    const rowHeight = 74;
    const preview = { x: innerWidth + 46, y: 44, width: 94, height: 150 };
    const x = d3.scaleLinear().domain([-7, 7]).range([0, innerWidth]);
    const rows = [
      { key: "shutter", label: "셔터", className: "exposure-shutter-bar" },
      { key: "aperture", label: "조리개", className: "exposure-aperture-bar" },
      { key: "iso", label: "ISO", className: "exposure-iso-bar" },
      { key: "total", label: "총합", className: "exposure-total-bar" },
    ];

    function setupSlider(input, options, selectedValue) {
    ...
    function formatStops(value) {
      if (Math.abs(value) < 0.05) return "0.0 stop";
      return `${value > 0 ? "+" : ""}${value.toFixed(1)} stops`;
    }

    function formatRatio(value) {
      if (value >= 100) return `${d3.format(".2s")(value)}배`;
      if (value >= 10) return `${value.toFixed(1)}배`;
      if (value >= 1) return `${value.toFixed(2)}배`;
      return `1/${(1 / value).toFixed(2)}배`;
    }

    function tradeoffText(shutter, aperture, iso) {
      const shutterNote = shutter >= 1 / 30 ? "움직임은 흔들리기 쉽다" : "움직임은 잘 멈춘다";
      const apertureNote = aperture <= 2.8 ? "배경 흐림이 커진다" : aperture >= 11 ? "초점 범위가 넓어진다" : "심도는 중간 정도다";
      const isoNote = iso >= 1600 ? "노이즈가 눈에 띌 수 있다" : "노이즈 부담은 작다";
      return `${shutterNote}. ${apertureNote}. ${isoNote}.`;
    }

    setupSlider(shutterInput, shutters, 1 / 500);
    setupSlider(apertureInput, apertures, 2.8);
    setupSlider(isoInput, isos, 400);

    chart.attr("viewBox", `0 0 ${width} ${height}`).selectAll("*").remove();
    const root = chart.append("g").attr("transform", `translate(${margin.left},${margin.top})`);
    root.append("g")
      .attr("class", "exposure-grid")
      .attr("transform", `translate(0,${innerHeight})`)
      .call(d3.axisBottom(x).tickValues(d3.range(-7, 8, 1)).tickSize(-innerHeight).tickFormat(""));
    root.append("g")
      .attr("class", "exposure-axis")
      .attr("transform", `translate(0,${innerHeight})`)
      .call(d3.axisBottom(x).tickValues(d3.range(-6, 7, 2)).tickFormat((d) => `${d > 0 ? "+" : ""}${d}`));
    root.append("line")
      .attr("class", "exposure-zero-line")
      .attr("x1", x(0))
      .attr("x2", x(0))
      .attr("y1", 0)
      .attr("y2", innerHeight);
    root.append("text")
      .attr("class", "exposure-label")
      .attr("x", innerWidth)
      .attr("y", innerHeight + 38)
      .attr("text-anchor", "end")
      .text("stops from ISO 100, f/4, 1/125s");

    const rowGroups = root.selectAll(".exposure-row")
      .data(rows)
      .join("g")
      .attr("class", "exposure-row")
      .attr("transform", (_d, index) => `translate(0,${index * rowHeight + 36})`);

    rowGroups.append("text")
      .attr("class", "exposure-row-label")
      .attr("x", -18)
      .attr("y", 5)
      .attr("text-anchor", "end")
      .text((d) => d.label);
    ...
    rowGroups.append("rect")
      .attr("class", (d) => `exposure-bar ${d.className}`)
      .attr("y", -13)
      .attr("height", 26)
      .attr("rx", 2);

    rowGroups.append("circle")
      .attr("class", "exposure-bar-end")
      .attr("r", 5);

    rowGroups.append("text")
      .attr("class", "exposure-stop-label")
      .attr("y", 5);

    const previewGroup = root.append("g")
      .attr("class", "exposure-preview")
      .attr("transform", `translate(${preview.x},${preview.y})`);
    previewGroup.append("rect")
      .attr("class", "exposure-preview-frame")
      .attr("width", preview.width)
      .attr("height", preview.height);
    previewGroup.append("rect")
      .attr("class", "exposure-preview-fill")
      .attr("x", 10)
      .attr("y", 10)
      .attr("width", preview.width - 20)
      .attr("height", preview.height - 20);
    previewGroup.append("circle")
      .attr("class", "exposure-preview-subject")
      .attr("cx", preview.width / 2)
      .attr("cy", 64)
      .attr("r", 24);
    previewGroup.append("text")
      .attr("class", "exposure-label")
      .attr("x", preview.width / 2)
      .attr("y", preview.height + 24)
      .attr("text-anchor", "middle")
      .text("recorded brightness");

    function update() {
      const shutterOption = shutters[Number(shutterInput.value)];
      const apertureOption = apertures[Number(apertureInput.value)];
      const isoOption = isos[Number(isoInput.value)];
      const shutter = shutterOption.value;
      const aperture = apertureOption.value;
      const iso = isoOption.value;
      const shutterStops = Math.log2(shutter / baseline.shutter);
      const apertureStops = -2 * Math.log2(aperture / baseline.aperture);
      const isoStops = Math.log2(iso / baseline.iso);
      const totalStops = shutterStops + apertureStops + isoStops;
      const lightRatio = Math.pow(2, totalStops);
      const data = [
        { key: "shutter", value: shutterStops },
        { key: "aperture", value: apertureStops },
        { key: "iso", value: isoStops },
        { key: "total", value: totalStops },
      ];
      const brightness = Math.max(12, Math.min(96, 24 + 22 * totalStops));
      const clampStop = (value) => Math.max(-7, Math.min(7, value));

      shutterLabel.value = shutterOption.label;
      apertureLabel.value = apertureOption.label;
      isoLabel.value = isoOption.label;

      rowGroups.data(rows.map((row) => ({ ...row, value: data.find((d) => d.key === row.key).value })));
      rowGroups.select(".exposure-bar")
        .transition()
        .duration(220)
        .attr("x", (d) => x(Math.min(0, clampStop(d.value))))
        .attr("width", (d) => Math.max(3, Math.abs(x(clampStop(d.value)) - x(0))));
      rowGroups.select(".exposure-bar-end")
        .transition()
        .duration(220)
        .attr("cx", (d) => x(clampStop(d.value)))
        .attr("cy", 0);
      rowGroups.select(".exposure-stop-label")
        .transition()
        .duration(220)
        .attr("x", (d) => d.value >= 0 ? Math.min(x(7) - 70, x(clampStop(d.value)) + 10) : Math.max(x(-7) + 70, x(clampStop(d.value)) - 10))
        .attr("text-anchor", (d) => d.value >= 0 ? "start" : "end")
        .text((d) => formatStops(d.value));

      previewGroup.select(".exposure-preview-fill")
        .transition()
        .duration(220)
        .attr("fill", `hsl(42 24% ${brightness}%)`);
      previewGroup.select(".exposure-preview-subject")
        .transition()
        .duration(220)
        .attr("fill", `hsl(205 22% ${Math.max(18, Math.min(88, brightness - 10))}%)`);

      values.innerHTML = `
        <div class="exponential-value-card">
          <strong>총 노출</strong>
          <span>${formatStops(totalStops)}</span>
          <span>기준의 ${formatRatio(lightRatio)}</span>
        </div>
        <div class="exponential-value-card">
          <strong>현재 설정</strong>
          <span>${shutterOption.label}, ${apertureOption.label}</span>
          <span>${isoOption.label}</span>
        </div>
        <div class="exponential-value-card">
          <strong>부수 효과</strong>
          <span>${tradeoffText(shutter, aperture, iso)}</span>
        </div>
      `;
    }

    shutterInput.addEventListener("input", update);
    apertureInput.addEventListener("input", update);
    isoInput.addEventListener("input", update);
    update();
  }
